...
local function harpoon_color()
    
    if require('harpoon.mark').get_index_of(vim.fn.bufname()) then
        return 'UserSLHarpoonYesMark'
    else
        return 'UserSLHarpoonNoMark'
    end
end
...
_G._generate_user_statusline_highlights = function()
  ...
  local groups = {
    ...
    UserSLHarpoonYesMark = { fg = pal.hint, bg = pal.sl.bg },
    UserSLHarpoonNoMark = { fg = pal.error, bg = pal.sl.bg },
  }
...
local c = {
  ...
  harpoon = {
    provider = function()
        local number = require('harpoon.mark').get_index_of(vim.fn.bufname())
        if number then
            return "ﯠ" .. number
        end
        return "ﯡ"
    ...
    hl = harpoon_color,
    right_sep = { str = " ", hl = harpoon_color },
  },
  file_enc = {
    provider = function()
